...
$$
where I have included only the terms involving the unknown parameter $\mu$. 
...
Make sure this distribution makes sense. In this case the distribution is on $\mu$ and its parameters are $\mu_{0}$ and $\tau$.
...
For lecture today you read the article by Fenton and Neil (2012) and the accompanying article from the Guardian.
...
Note that if p=$10^{-6}$, then $P(\mbox{guilt}|\mbox{match})$=0.5. If p=$0.5$, then $P(\mbox{guilt}|\mbox{match})$=0.999999. So, your decision about the guilt or innocence of the defendent depends a lot on your prior expectations for their guilt of innocence.
</span>
</details> 
