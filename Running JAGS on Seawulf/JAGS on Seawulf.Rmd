...
Throughout these instructions, I use NAME to refer to your login name on the system.

# Transferring files to Seawulf

For the examples below, you will need several files in your home directory. Open Terminal on your local machine. Note that transferring files is done FROM your local machine. In other words, do not log into Seawulf and then try and transfer files. You need to be logged into your local machine, with Terminal (or equivalent) and then issue the following commands to transfer files from your local machine to the remote machine. LOCATION refers to the location on your local machine where the files currently reside.
...
We'll start off by opening an interactive window to access R on Seawulf. This is not something we would do in regular practice, but knowing how to do it is useful when things are not working, or you are getting errors. We'll start from the very beginning. In a Terminal
...
We can either do this directly in the terminal by using the command `mkdir R_libs`, or in R. In the example script `TestJAGSCode.R` this is done in the line
...
However if you have a lot of packages it's easier to tell R where to find the packages when it loads.
